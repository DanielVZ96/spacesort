...
pub fn cribench_sorts_max_range(c: &mut Criterion) {
    ...
    let mut group = c.benchmark_group("sorts_v2_max_range");
    ...
    for size in build_vec(MAX_SIZE).iter() {
        ...
        group.bench_with_input(BenchmarkId::new("v2", size), size, |b, &size| {
            b.iter(|| black_box(sort_v2(values.clone())));
        });
...
pub fn cribench_sorts_med_range(c: &mut Criterion) {
    let mut group = c.benchmark_group("sorts_v2_med_range");
    let plot_config = PlotConfiguration::default()
        .summary_scale(AxisScale::Logarithmic);
    group.plot_config(plot_config);
    let rng = Pcg64::from_seed([2;32]);
...
pub fn cribench_sorts_small_range(c: &mut Criterion) {
    let mut group = c.benchmark_group("sorts_v2_small_range");
    let plot_config = PlotConfiguration::default()
        .summary_scale(AxisScale::Logarithmic);
    group.plot_config(plot_config);
    let rng = Pcg64::from_seed([2;32]);
